...
class SimpleEquationDiscretizer2:
	...
	# Get the coordinates of the variable around which the row-th row is created
	def getCoordinates2(self, row):
	    return int(row / (self.N + 1)), row % (self.N + 1)


	# Get the row of a(i, j)'s equation
	def getRow2(self, i, j):
	    return(i * (self.N + 1) + j)
	...
	def computeMatrixAndVector(self):
	    ...
	    for currentRow in range((self.N + 1) * (self.N + 1)):
	    	(xx,yy) = self.getCoordinates2(currentRow)
	    	if(not(self.isOnBorder(xx,yy))):
	    		print(xx,yy)
	    		nextRow = nextRow + 1
	        	self.computeRow(currentRow, nextRow)


	# Compute the elements of row-th row in (rowList, colList, dataList) for -nabla f(x,t) = g(x,t) problem
	def computeRow(self, row, nextRow):
	    (x, y) = self.getCoordinates2(row)
	    print(nextRow)
	    value = - self.valueFunction((1.0) * x / self.N, (1.0) * y / self.N) * self.h * self.h
	    self.addEntryToMatrices(nextRow, nextRow, 4.0)

	    for (dX, dY) in [(1, 0), (-1, 0), (0, 1), (0, -1)]:
	        if(not(self.isOnBorder(x + dX, y + dY))):
	            self.addEntryToMatrices(nextRow, self.getRow(x + dX - 1, y + dY - 1), -1.0)
	        else:
